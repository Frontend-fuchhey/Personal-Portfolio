...
const photoData = [{ id: 1, url: shrawanPic, title: "Moment 1" }];
...
export const PhotosApp = memo(() => {
  ...
  return (
    <div
      className={`h-full w-full flex flex-col bg-white dark:bg-[#0f172a] font-sans relative ${isMobile ? "mobile-content-shift" : ""}`}
      style={{ zIndex: 10 }}
    >
      {/* Gallery Header */}
      {!isMobile && (
        <div className="flex items-center gap-2 px-6 py-4 border-b border-gray-100 dark:border-gray-800">
          ...
          <h2 className="text-sm font-bold text-gray-700 dark:text-gray-200 uppercase tracking-widest">
            Memories Gallery
          </h2>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-3 md:p-6">
        <div
          className={`grid ${isMobile ? "grid-cols-3 gap-1.5" : "grid-cols-4 gap-4"}`}
        >
...
const PhotoItem = memo(
  ({ photo, onClick }: { photo: any; onClick: () => void }) => {
    ...
    return (
      ...
      >
        ...
        <div
          className={`absolute inset-0 bg-white/5 animate-pulse ${loaded ? "opacity-0" : "opacity-100"} transition-opacity duration-300`}
        />
