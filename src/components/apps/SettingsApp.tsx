  ...
  return (
    <div className="h-full w-full overflow-y-auto p-6 bg-white/70 backdrop-blur-2xl font-sans flex flex-col gap-8">
      ...
      <section>
        ...
        <div
          className={`grid gap-3 mb-4 ${hasCustomWallpaper ? "grid-cols-4" : "grid-cols-3"}`}
        >
            ...
            <button
              ...
              className={`relative h-20 rounded-xl overflow-hidden shadow-sm transition-transform active:scale-95 cursor-pointer ${
                wallpaper.id === wp.id
                  ...
                  : "hover:opacity-90 outline-1 outline-black/5 hover:outline-black/10"
              }`}
              style={
                wp.type === "url"
                  ? {
                      backgroundImage: `url(${wp.url || wp.value})`,
      ...
      <section>
        ...
        <div className="bg-white/80 rounded-2xl shadow-sm border border-gray-200/50 overflow-hidden">
          <div className="flex items-center justify-between p-4 border-b border-gray-200/60">
            ...
            >
              <div
                className={`absolute top-0.5 bottom-0.5 w-5 rounded-full bg-white shadow-sm transition-all duration-300 ease-spring ${showClock ? "left-6" : "left-0.5"}`}
              />
            </button>
          </div>
          ...
          <div className="flex items-center justify-between p-4">
            ...
            >
              <div
                className={`absolute top-0.5 bottom-0.5 w-5 rounded-full bg-white shadow-sm transition-all duration-300 ease-spring ${reduceMotion ? "left-6" : "left-0.5"}`}
              />
            </button>
          </div>
        </div>
